...
Mock.mock(RegExp(`${domain}/admin/getMaterailTable.*`), 'get', (options: Options) => {
	...
	return {
		...
		'code': 1,
		'data': {
			table_data: materailList.slice(obj.params.page_size * (obj.params.page_number - 1), obj.params.page_size * obj.params.page_number),
...
Mock.mock(RegExp(`${domain}/admin/saveMaterails.*`), 'post', (options: Options) => {
	...
	const result = obj.body.file_list.map((file: any) => {
		file.level_one_id = obj.body.level_one_id
		file.level_two_id = obj.body.level_two_id
		const v1 = treeList.find(l => l.id === file.level_one_id)
		if (v1) {
			file.level_one_name = v1.level_name
			const v2 = v1.children.find(l => l.id === file.level_two_id)
			if (v2) {
